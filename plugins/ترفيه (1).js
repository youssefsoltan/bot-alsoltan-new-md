let handler = m => m 
 ...
 if (/^منور|نورت$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, ` *بنورك اكيد❤️* `, m) //wm, null, [['Menu', '#menu']], m) botones :V 
 ...
 if (/^جيت$/i.test(m.text) ) { //sem prefixo 
 ...
     
 if (/^عمك|مين عمك$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, `يوسف السلطان عمي اكيد`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
